...
// Schema embedded inline — no fs.readFileSync needed, so this works after tsc compilation.
...
/**
 * Exported function — called from index.ts on every server boot.
 * Safe to run repeatedly because all statements use IF NOT EXISTS.
 */
export async function runMigration(): Promise<void> {
  await testConnection();
  runRawSql(SCHEMA_SQL);
